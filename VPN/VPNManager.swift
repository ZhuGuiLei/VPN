...
class VPNManager: NSObject {
    ...
    init(type: VPNProtocolType, VPNName: String?, serverAddress: String?, remoteIdentifier: String?, passwordReference: String?, sharedSecretReference: String?) {
        super.init()
        ...
        vpnMgr.loadFromPreferences { (error) in
            if let e = error {
                ...
                do {
                    ...
                    SVProgressHUD.showSuccess(withStatus: "连接...")
                } catch {
    ...
    func disconnectVPN() {
        vpnMgr.loadFromPreferences { (error) in
            if let e = error {
                ...
                SVProgressHUD.showSuccess(withStatus: "断开成功")
            }
        }
    }
